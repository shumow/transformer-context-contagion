...
size ladder shows the copy is \emph{induction-head-mediated at every scale} (124M--6.9B, GPT-2
and Pythia): ablating the top few percent of heads by induction score collapses reproduction,
...
E2 attribution at scale & done & induction heads carry the knee to 6.9B, GPT-2+Pythia (\S\ref{sec:e2scale}). \\
\bottomrule
\end{tabular}
...
is now done (E2 to 6.9B, \S\ref{sec:e2scale}); the coherent self-extraction capstone (E8) remains---the
24\,GB-GPU stage.
...
\subsection{E2 at scale: the mechanism holds to 6.9B, across families}\label{sec:e2scale}
...
\textbf{C2 holds across scale and family (124M--6.9B, GPT-2 and Pythia).} Ablating the top
$\sim\!5\%$ of heads by induction score collapses reproduction near the knee ($-70\%$ to
$-98\%$ at $p\ge3$), while an equal-sized random ablation barely dents it
...
via HookedTransformer, fp16; Qwen2.5-7B did not load, so no third family at 7B; 4 payloads,
single seed, greedy.)
...
reproduction across scale and family (124M--6.9B, GPT-2 and Pythia), so \textbf{C2 is now
