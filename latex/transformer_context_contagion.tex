...
nested substitution into text and self-executing a decoded benign instruction (E8). What remains
is the larger-context, retrieval-grounded versions of the downstream claims.
\end{abstract}
...
mechanistically established at scale} (\S\ref{sec:e2scale}), not just on GPT-2-small. What is
\emph{not} yet established: the worm's critical length and induction range at scale, the
larger-context retrieval-grounded versions of C5 and C6, and a real-task (not naturalness-proxy)
usefulness for C4. The self-extraction capstone (E8, \S\ref{sec:e8}) is now done---capable
instruct models unfold nested substitution into coherent text and self-execute a decoded benign
instruction, far past the toy's depth limit, while the surface stays gibberish---so what remains
is the larger-context, retrieval-grounded work.
