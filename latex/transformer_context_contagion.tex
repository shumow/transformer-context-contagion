...
What remains is scale: a genuine size sweep and instruction-tuned models are capped by the
8\,GB development machine. This document will be revised as those land.
\end{abstract}
...
E1.4 modern / chat models & \tbd & base vs.\ instruct (needs $>$8\,GB). \\
E1.3 size sweep ($\ge$355M) & \tbd & how the knee moves with scale (needs $>$8\,GB). \\
\bottomrule
\end{tabular}
...
result. What is \emph{not} yet established is \textbf{scale}: how the knee, the worm's critical
length, and the induction range move with model size (the sweep is capped at 162M by 8\,GB), and
whether instruction-tuned models behave the same---plus the larger-context, retrieval-grounded
versions of C5 and C6, and a real-task (not naturalness-proxy) usefulness for C4. Those need more
than an 8\,GB machine and are the next stage.
