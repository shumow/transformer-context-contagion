...
interpretability tooling. The mechanistic attribution (activation patching), the
model-size scaling, the delivery/hijack experiments, and the cross-model worm remain
\tbd. This document will be revised as those land.
\end{abstract}
...
E1.3 model-size sweep & \tbd & needs more compute than the dev laptop. \\
E1.4 modern / chat models & \tbd & base vs.\ instruct. \\
E2 induction attribution & \tbd & activation patching / head ablation (mechanistic C2). \\
...
(\S\ref{sec:e1}) and behavioral evidence on \textbf{C2} (\S\ref{sec:e1b}); C4--C6 are \tbd.}
\label{tab:claims}
\end{table}
...
(\tbd); \textbf{context-length vs.\ $N$}---a moving knee could be a context-length effect, to
be checked by holding total context fixed (\tbd); and \textbf{scale}---induction strengthens
with model size, so the knee is expected to move, which is the point of the size sweep
(\tbd).
...
string length, the induction-like length-robustness predicted by the toy (C3); and
reproduction is predominantly a cue-triggered copy rather than a frequency bias, a
behavioral induction signature (C2, behaviorally). What is \emph{not} yet established: the
mechanistic attribution to induction heads (E2), how any of this scales with model size
(E1.3) or survives instruction tuning (E1.4), and the entire delivery (C5) and transmission
(C6) story---whether a planted span hijacks a realistic long context, and whether output fed
back propagates with a reproduction number that falls with length. Those are the next
stages, and this report will grow to cover them.
